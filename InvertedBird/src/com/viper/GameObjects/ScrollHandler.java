...
public class ScrollHandler {
	...
	public ScrollHandler(GameWorld gameWorld, float yPos) {
		...
		pipe1 = new Pipe(-240, 0, 22, 60, SCROLL_SPEED, yPos);
		pipe2 = new Pipe((pipe1.getTailX() - PIPE_GAP), 0, 22, 70, SCROLL_SPEED,
				yPos);
		pipe3 = new Pipe((pipe2.getTailX() - PIPE_GAP), 0, 22, 60, SCROLL_SPEED,
				yPos);
	...
	public void update(float delta) {
		...
		if (pipe1.isScrolledRight()) {
			pipe1.reset(pipe3.getTailX() - PIPE_GAP);
		} else if (pipe2.isScrolledRight()) {
			pipe2.reset(pipe1.getTailX() - PIPE_GAP);

		} else if (pipe3.isScrolledRight()) {
	...
	public boolean collides(Bird bird) {
		...
		} else if (!pipe2.isScored()	&& pipe2.getX() + (pipe2.getWidth() / 2) < bird.getX()
				+ bird.getWidth()) {
					addScore(1);
					pipe2.setScored(true);
					AssetLoader.coin.play();

		} else if (!pipe3.isScored()	&& pipe3.getX() + (pipe3.getWidth() / 2) < bird.getX()
				+ bird.getWidth()) {
					addScore(1);
...
if (!pipe1.isScored() && pipe1.getX() + (pipe1.getWidth() / 2) > bird.getX() + bird.getWidth()) {
					addScore(1);
					pipe1.setScored(true);
					AssetLoader.play(AssetLoader.coin);
	...
	public void onRestart() {
		...
		backGrass.onRestart(0, SCROLL_SPEED);
		frontGrass.onRestart(backGrass.getTailX(), SCROLL_SPEED);
