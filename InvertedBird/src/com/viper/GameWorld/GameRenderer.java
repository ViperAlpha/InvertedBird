...
import com.viper.GameObjects.Grass;
import com.viper.GameObjects.Pipe;
import com.viper.GameObjects.ScrollHandler;
import com.viper.IBHelpers.AssetLoader;
import com.viper.IBHelpers.InputHandler;
import com.viper.InvertedBird.IBGame;
import com.viper.TweenAccessors.Value;
import com.viper.TweenAccessors.ValueAccessor;
import com.viper.ui.SimpleButton;
import com.viper.ui.SimpleButton.Type;
...
public class GameRenderer 
{
	...
	IBLogo, gameOver, highScore, scoreboard, star, noStar, retry;
	private Animation birdAnimation,birdAnimationMENU;
	...
	private void drawAds(boolean show){
		IBGame.myRequestHandler.showAds(show);
	}
	...
	private void initAssets() 
		{
			...
			birdAnimationMENU = AssetLoader.birdAnimationMENU;
			birdMid = AssetLoader.bird;
			skullUp = AssetLoader.skullUp;
			skullDown = AssetLoader.skullDown;
			bar = AssetLoader.bar;
			ready = AssetLoader.ready;
			IBLogo = AssetLoader.ibLogo;
	...
	private void drawBirdCentered(float runTime) 
		{
			batcher.draw(birdAnimationMENU.getKeyFrame(runTime), 106, midPointY - 15,
	...
	private void drawButtons(){
		for (SimpleButton button : menuButtons) {
			if(myWorld.isReady() && button.getType()==Type.Activable){
				button.draw(batcher);
			}
	...
	private void drawMenuUI() 
		{
			float gameWidth = myWorld.getGameWidth();
			float IBlogoWidth = IBLogo.getRegionWidth();
			batcher.draw(IBLogo, (gameWidth/2) - ((IBlogoWidth/2.0f)/2), midPointY - 50,
					IBlogoWidth / 2.0f, IBLogo.getRegionHeight() / 2.0f);
		}
	...
	private void drawRetry() {
		batcher.draw(retry, 36, midPointY + 10, 66, 17);
	}

	private void drawReady() {
		batcher.draw(ready, 36, midPointY - 50, 62, 17);
	}

	private void drawGameOver() {
		batcher.draw(gameOver, 36, midPointY - 55, 67, 19);
	}
	...
	private void drawSensors(){
		int inclination = (int) Math.round(Math.toDegrees(Math.acos(Gdx.input.getAzimuth())));
		int x =15; int y= 15;

		batcher.begin();

		AssetLoader.font.setScale(.13f, -.13f);

		String str = "Incli " + Float.toString(inclination);
		AssetLoader.font.draw(batcher, str, x, y); y+=15;

		str = "X :" + Gdx.input.getAccelerometerX();
		AssetLoader.font.draw(batcher, str, x, y);y+=15;

		str = "Y :"+ Gdx.input.getAccelerometerY();
		AssetLoader.font.draw(batcher, str, x, y); y+=15;

		str = "Z : "+ Gdx.input.getAccelerometerZ();
		AssetLoader.font.draw(batcher, str, x, y); y+=15;

		str = "Azi :" + Gdx.input.getAzimuth();
		AssetLoader.font.draw(batcher, str, x, y); y+=15;
	...
	public void render(float delta, float runTime) {
		...
		//		shapeRenderer.setColor(73/255.0f, 196/255.0f, 204/255.0f, 1);
		shapeRenderer.setColor(55 / 255.0f, 80 / 255.0f, 100 / 255.0f, 1);
		shapeRenderer.rect(0, 0, 136, midPointY + 66);

		// Draw Grass
		//		shapeRenderer.setColor(197 / 255.0f, 171 / 255.0f, 77 / 255.0f, 1);
		shapeRenderer.setColor(111 / 255.0f, 186 / 255.0f, 45 / 255.0f, 1);
		shapeRenderer.rect(0, midPointY + 66, 136, 11);

		// Draw Dirt
		shapeRenderer.setColor(147 / 255.0f, 80 / 255.0f, 27 / 255.0f, 1);
		//		shapeRenderer.setColor(222 / 255.0f, 218 / 255.0f, 145 / 255.0f, 1);
		...
		batcher.draw(bg, 0, midPointY + 13, 136, 53);
		...
		if (myWorld.isRunning()) {
			...
			drawScore();
			if(AccelerometerY < -2.0){
				AssetLoader.font.draw(batcher,"Don't cheat.",5,30);
			}
			else
				{
				drawBird(runTime);
	...
	public void drawCollision(float delta)
		{
			shapeRenderer.begin(ShapeType.Filled);
